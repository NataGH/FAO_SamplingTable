...
setwd("C:/Users/natalia/Documents/FAO/script/var_colonne/Italy_2006")
dir()

muTab <- read.csv("ExpectedValues_t_Italy_2006.csv",row.names=1,sep=",")[-105,-c(8:10)]
dim(muTab)

head(muTab)
tail(muTab)

n0<-read.csv("ExpectedValues_t_Italy_2006.csv",row.names=1,sep=",")[-105,9]
n0



muCol<-read.csv("ExpectedValues_t_Italy_2006.csv",row.names=1,sep=",")[105,-c(8:10)]
muCol


bounds <- array(NA,c(dim(muTab),2))
dimnames(bounds) <- c(dimnames(muTab),list(c("Lower","Upper")))
bounds[,,1] <- as.matrix(read.csv("LowerBounds_t_Italy_2006.csv",row.names=1,sep=','))[-105,]
bounds[,,2] <- as.matrix(read.csv("UpperBounds_t_Italy_2006.csv",row.names=1,sep=','))[-105,]
bounds[1:5,,]



lowCol<-read.csv("LowerBounds_t_Italy_2006.csv",row.names=1,sep=',')[105,]
lowCol

uppCol<-read.csv("UpperBounds_t_Italy_2006.csv",row.names=1,sep=',')[105,]
uppCol
...
Scenario <- read.csv("Scenario_t_Italy_2006.csv",sep=",",header=T,row.names=1)[-105,-(8:10)]


rmse <- do.call(rbind,lapply(1:dim(tab[[1]])[3],function(t)sqrt(mean(((tab[[1]][,,t]-Scenario))^2))))
rmse

rrmse <- do.call(rbind,lapply(1:dim(tab[[1]])[3],function(t)sqrt(mean(((tab[[1]][,,t]-Scenario)/(Scenario+0.0000000001))^2))))
rrmse
