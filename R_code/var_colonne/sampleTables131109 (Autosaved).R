...
den <- 1:3
reject <- rmse <- rrmse <- time <- rep(NA,length(den))
...
#	controlCol <- do.call(cbind,lapply(1:ncol(muTab),function(j)range(round(rnorm(10000,colSums(muTab),sqrt(colSums(sdTab^2))/den[i])))))
	...
	rmse[i] <- do.call(rbind,lapply(1:dim(tab[[1]])[3],function(t)sqrt((sum((tab[[1]][,,t]-Scenario)^2))/(length(tab[[1]][,,t])))))
	rrmse[i] <- do.call(rbind,lapply(1:dim(tab[[1]])[3],function(t)sqrt((sum(((tab[[1]][,,t]-Scenario)/(Scenario+0.0000000001))^2))/(length(tab[[1]][,,t])))))
